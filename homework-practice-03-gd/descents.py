...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray):
        ...
        # MSE(w, x, y) = 1/l(y - xw)^t(y - xw)
        print("Shape of x:", x.shape)
        print("Shape of y:", y.shape)
        y_sparse = sparse.csr_matrix(y.reshape(-1, 1))

    
        D = y_sparse - np.dot(x, self.w)
        res = np.dot(D.T, D)
        return res / (np.shape(y)[0])
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray):
        indexes = np.random.randint(np.shape(y)[0], size=self.batch_size)
        x_batch = x[indexes]  
        print("Shape of x_batch:", x_batch.shape)  
        print("Shape of self.w:", self.w.shape)  
        D = y[indexes] - np.dot(x_batch, self.w)
        res = np.dot(x_batch.T, D)
        return -2 * res / self.batch_size
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray):
        ...
        self.v *= self.beta_2
        self.v += (1 - self.beta_2) * (gradient * gradient)

        k1 = (1 - self.beta_1**self.iteration)
        k2 = (1 - self.beta_2**self.iteration)    
        diff = -self.lr() * (self.m / k1) / (np.sqrt(self.v / k2) + self.eps)
        self.w += diff
        return diff
